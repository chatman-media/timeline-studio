...
use crate::analysis::types::{
    AudioFloat, AudioVolume, AudioDuration, AudioAnalysisError,
    AudioMontageAnalysis, AudioBasicMetrics,
};
use crate::montage_planner::types::{
    EmotionalTone, AudioContentType
};
use anyhow::Result;
...
impl UnifiedMontageAudioAnalyzer {
    ...
    pub async fn analyze_audio<P: AsRef<Path>>(
        ...
        info!("Starting unified montage audio analysis for: {}", path.display());
        ...
        let basic_metrics = self.extract_basic_metrics(path).await
            .map_err(|e| AudioAnalysisError::ProcessingError(
                format!("Failed to extract basic metrics: {}", e)
            ))?;

        debug!("Basic metrics extracted: duration={}s, channels={}", 
        ...
        info!("Unified montage audio analysis completed in {}ms", processing_time.as_millis());
    ...
    pub async fn analyze_audio_segment<P: AsRef<Path>>(
        ...
        debug!("Analyzing unified audio segment: {}s-{}s", start_time, start_time + duration);

        // Extract segment features using unified FFmpeg analysis
        let segment_analysis = self.extract_segment_features_unified(path, start_time, duration).await?;
    ...
    fn estimate_audio_quality(&self, sample_rate: u32, channels: u32, duration: AudioFloat) -> AudioFloat {
    ...
    async fn detect_speech_presence_unified<P: AsRef<Path>>(
        ...
        let basic_metrics = self.extract_basic_metrics(audio_path).await
            .map_err(|e| AudioAnalysisError::ProcessingError(e.to_string()))?;

        let total_silence_duration: AudioFloat = silence_periods.iter()
            .map(|p| p.duration.seconds)
            .sum();
    ...
    async fn detect_beats_unified<P: AsRef<Path>>(
        ...
        if let Some(tempo) = tempo_opt {
            let basic_metrics = self.extract_basic_metrics(path.as_ref()).await
                .map_err(|e| AudioAnalysisError::ProcessingError(e.to_string()))?;
        ...
        let output = AsyncCommand::new("ffmpeg")
            .args([
                "-i", path.to_str().ok_or_else(|| 
                    AudioAnalysisError::ProcessingError("Invalid path".to_string()))?,
                "-af", "volumedetect",
                "-f", "null", "-",
            ])
            .output()
    ...
    async fn calculate_dynamic_range_unified<P: AsRef<Path>>(
        ...
        let output = AsyncCommand::new("ffmpeg")
            .args([
                "-i", audio_path.to_str().ok_or_else(|| 
                    AudioAnalysisError::ProcessingError("Invalid path".to_string()))?,
    ...
    async fn calculate_ambient_level_unified<P: AsRef<Path>>(
        ...
        if !silence_periods.is_empty() {
            let avg_noise: AudioFloat = silence_periods.iter()
                .map(|p| p.noise_level.level)
                .sum::<AudioFloat>() / silence_periods.len() as AudioFloat;
    ...
    async fn detect_tempo_ffmpeg_unified<P: AsRef<Path>>(
        ...
        let _output = AsyncCommand::new("ffmpeg")
            .args([
                ...
                "-af", "highpass=f=100,lowpass=f=5000,aresample=22050,astats=metadata=1:reset=1",
                "-f", "null", "-",
            ])
            .output()
            .await?;
    ...
    async fn analyze_spectral_content_unified<P: AsRef<Path>>(
        ...
        let output = AsyncCommand::new("ffmpeg")
            .args([
                "-i", path.to_str().ok_or_else(|| 
                    AudioAnalysisError::ProcessingError("Invalid path".to_string()))?,
                "-af", "astats=metadata=1:measure_perchannel=none",
                "-f", "null", "-",
            ])
            .output()
            .await
    ...
    pub fn to_legacy_montage_analysis(&self, result: &UnifiedMontageAudioAnalysisResult) -> AudioMontageAnalysis {
        ...
        let beat_detection = result.tempo.map(|tempo| {
            crate::analysis::types::AudioBeatAnalysis {
                bpm: tempo,
                beat_times: result.beat_markers.iter().map(|&t| 
                    crate::analysis::types::AudioTimestamp::from_seconds(t)
                ).collect(),
...
mod tests {
    ...
    fn test_quality_score_calculation() {
        ...
        let quality_score = analyzer.calculate_quality_score(&basic_metrics, &energy_level, dynamic_range);
