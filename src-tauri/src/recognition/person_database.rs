...
impl PersonDatabase {
    ...
    pub async fn get_person(&self, person_id: Uuid) -> Result<Option<PersonProfile>> {
        ...
        let person = stmt.query_row([person_id.to_string()], |row| {
            Ok(PersonProfile {
    ...
    pub async fn find_similar_persons(
        ...
        let rows = stmt.query_map(rusqlite::params![self.quality_threshold], |row| {
            ...
            let embedding = self.deserialize_embedding(&embedding_bytes)
                .map_err(|_| rusqlite::Error::InvalidColumnType(3, "embedding".to_string(), rusqlite::types::Type::Blob))?;
            let similarity = cosine_similarity(target_embedding, &embedding);

            Ok((person_id, name, embedding, quality, appearance_count as u64, similarity))
        })?;
    ...
    pub async fn get_stats(&self) -> Result<DatabaseStats> {
        ...
        let total_persons: u64 = conn.query_row("SELECT COUNT(*) FROM persons", [], |row| row.get(0))?;
        let total_embeddings: u64 = conn.query_row("SELECT COUNT(*) FROM face_embeddings", [], |row| row.get(0))?;
        let total_appearances: u64 = conn.query_row("SELECT COUNT(*) FROM person_appearances", [], |row| row.get(0))?;
        let total_thumbnails: u64 = conn.query_row("SELECT COUNT(*) FROM person_thumbnails", [], |row| row.get(0))?;
        ...
        let average_quality_score: f32 = conn.query_row(
            "SELECT AVG(quality_score) FROM face_embeddings", [], |row| row.get(0)
        ).unwrap_or(0.0);
    ...
    pub async fn get_face_embeddings_for_person(&self, person_id: &Uuid) -> Result<Vec<FaceEmbedding>> {
        let conn = self.conn.lock().await;
        let mut stmt = conn.prepare(
        ...
        let embeddings = stmt.query_map([person_id.to_string()], |row| {
            ...
            Ok(FaceEmbedding {
                ...
                embedding: self.deserialize_embedding(&embedding_bytes).map_err(|_e| rusqlite::Error::InvalidColumnType(0, "embedding".to_string(), rusqlite::types::Type::Blob))?,
                quality_score: row.get(3)?,
                head_pose: serde_json::from_str(&head_pose_str).map_err(|_| rusqlite::Error::InvalidColumnType(3, "head_pose".to_string(), rusqlite::types::Type::Text))?,
                emotion: if emotion_str.is_empty() { None } else { Some(serde_json::from_str(&emotion_str).map_err(|_| rusqlite::Error::InvalidColumnType(4, "emotion".to_string(), rusqlite::types::Type::Text))?) },
                demographics: if demographics_str.is_empty() { None } else { Some(serde_json::from_str(&demographics_str).map_err(|_| rusqlite::Error::InvalidColumnType(5, "demographics".to_string(), rusqlite::types::Type::Text))?) },
                lighting_conditions: serde_json::from_str(&lighting_conditions_str).map_err(|_| rusqlite::Error::InvalidColumnType(6, "lighting_conditions".to_string(), rusqlite::types::Type::Text))?,
                source_clip_id: Uuid::parse_str(&row.get::<_, String>(8)?).unwrap(),
                frame_number: row.get(9)?,
                timestamp: row.get(10)?,
                bounding_box: serde_json::from_str(&bounding_box_str).map_err(|_| rusqlite::Error::InvalidColumnType(11, "bounding_box".to_string(), rusqlite::types::Type::Text))?,
