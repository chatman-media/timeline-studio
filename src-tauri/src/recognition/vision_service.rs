...
impl VisionService {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        if self.config.enable_object_detection {
            ...
            match YoloProcessor::new(self.config.yolo_model.clone(), self.config.object_confidence) {
                Ok(processor) => {
                    info!("YOLO processor initialized successfully");
                    *yolo_lock = Some(processor);
                }
                Err(e) => {
                    warn!("Failed to initialize YOLO processor: {}. Object detection will be unavailable.", e);
                }
            }
        }
            ...
            match RetinaFaceProcessor::new(self.config.retinaface_model.clone()) {
                Ok(mut processor) => {
                    ...
                    if let Err(e) = processor.load_model().await {
                        warn!("Failed to load RetinaFace model: {}. Face detection will be unavailable.", e);
                    } else {
                ...
                Err(e) => {
                    warn!("Failed to initialize RetinaFace processor: {}. Face detection will be unavailable.", e);
                }
            }
        }
        ...
        if self.config.enable_face_embeddings {
            ...
            match FaceNetProcessor::new(self.config.facenet_model.clone()) {
                Ok(mut processor) => {
                    // Load model async
                    if let Err(e) = processor.load_model().await {
                        warn!("Failed to load FaceNet model: {}. Face embeddings will be unavailable.", e);
                    } else {
                ...
                Err(e) => {
                    warn!("Failed to initialize FaceNet processor: {}. Face embeddings will be unavailable.", e);
                }
            }
        }
    ...
    pub async fn detect_objects(&self, image_path: &str) -> Result<Vec<ObjectDetection>> {
        ...
        let detections = processor.process_image(Path::new(image_path)).await
    ...
    pub async fn detect_faces(&self, image_path: &str) -> Result<Vec<FaceDetection>> {
        ...
        let detections = processor.detect_faces_sync(&img)
    ...
    pub async fn extract_face_embedding(&self, image_path: &str, bbox: &BoundingBox) -> Result<Vec<f32>> {
        ...
        // Generate embedding
        let embedding = processor.generate_embedding_sync(&face_img)
    ...
    pub async fn detect_faces_with_embeddings(&self, image_path: &str) -> Result<Vec<FaceDetection>> {
        ...
        for face in &mut faces {
            match self.extract_face_embedding(image_path, &face.bounding_box).await {
                Ok(embedding) => {
