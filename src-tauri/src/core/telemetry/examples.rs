...
use crate::core::telemetry::{
  AlertRule, AlertSeverity, MetricsExporter, TelemetryConfig,
  TelemetryManager, TracingExporter,
};
...
pub fn prometheus_config() -> TelemetryConfig {
  ...
  config.tracing.exporters = vec![TracingExporter::Jaeger { endpoint: "http://localhost:14268/api/traces".to_string() }];
...
pub fn jaeger_config() -> TelemetryConfig {
  ...
  config.tracing.exporters = vec![TracingExporter::Jaeger { endpoint: "http://localhost:14268/api/traces".to_string() }, TracingExporter::Console];
  config.metrics.enabled = true;
...
pub async fn setup_production_alerts(telemetry: &TelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
...
pub async fn metrics_example(telemetry: &TelemetryManager) {
  ...
  // Счетчик запросов
  let _ = metrics.increment_counter("http_requests_total", &[("method".to_string(), "GET".to_string()), ("status".to_string(), "200".to_string())]);
  
  // Гистограмма времени ответа
  let _ = metrics.record_histogram("http_request_duration_seconds", 0.125, &[("method".to_string(), "GET".to_string())]);
...
pub async fn tracing_example() {
  ...
  let span = tracing::info_span!("example_operation", user_id = 123, operation = "data_processing");
...
mod tests {
  ...
  fn test_prometheus_config() {
    ...
    assert!(config.metrics.exporters.contains(&MetricsExporter::Prometheus { port: 9090 }));
  }
  ...
  fn test_jaeger_config() {
    ...
    assert!(config.tracing.exporters.iter().any(|e| matches!(e, TracingExporter::Jaeger { .. })));
