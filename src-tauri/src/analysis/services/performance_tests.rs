...
mod performance_tests {
    ...
    use crate::analysis::types::{AudioPerformanceMode, UnifiedAudioConfig};
    ...
    async fn test_performance_fast_mode() {
        ...
        match result {
            Ok(analysis) => {
                ...
                println!("Engines used: {:?}", analysis.analysis_metadata.engines_used);
                println!(
                    "Success rate: {:.1}%",
                ...
                println!(
                    "Has audio: {}",
                    analysis.basic_metrics.has_audio
                );
                println!(
                    "Duration: {:.2}s",
                    analysis.basic_metrics.duration.seconds
                );
    ...
    async fn test_recommended_configs() {
        ...
        println!(
            "  Max time: {:?}s",
            fast_config.max_processing_time_seconds
        );
