...
impl SceneDetector {
    ...
    pub async fn detect_scenes(&self, file_path: &str) -> Result<Vec<AnalysisScene>> {
        ...
        let file_metadata = self.extract_file_metadata(file_path).await
            .context("Failed to extract file metadata")?;

        info!("File duration: {:.2}s, analyzing scenes...", file_metadata.duration);

        // Выполняем детекцию смены сцен через FFmpeg
        let scene_changes = self.detect_scene_changes(file_path).await
            .context("Failed to detect scene changes")?;

        info!("Detected {} scene changes", scene_changes.len());

        // Создаем сцены на основе точек смены
        let scenes = self.create_scenes_from_changes(
            &file_metadata,
            &scene_changes,
            file_path,
        ).await?;
    ...
    async fn extract_file_metadata(&self, file_path: &str) -> Result<FileMetadata> {
        ...
        let estimated_duration = (file_size as f32 / 1_000_000.0).max(10.0).min(3600.0); // 1MB ≈ 1 секунда
    ...
    async fn detect_scene_changes(&self, file_path: &str) -> Result<Vec<f32>> {
        debug!("Detecting scene changes with threshold: {}", self.scene_change_threshold);
    ...
    async fn create_scenes_from_changes(
        ...
        for (i, &start_time) in scene_changes.iter().enumerate() {
            ...
            if duration < self.min_scene_duration {
                debug!("Skipping scene {:.2}-{:.2}s (too short: {:.2}s)", start_time, end_time, duration);
                continue;
            }

            debug!("Analyzing scene {:.2}-{:.2}s (duration: {:.2}s)", start_time, end_time, duration);

            // Анализируем качество и характеристики сцены
            let scene_analysis = self.analyze_scene_segment(
                file_path,
                start_time,
                end_time,
                metadata,
            ...
            let scene = AnalysisScene {
                ...
                user_rating: None,
                representative_frame: scene_analysis.representative_frame,
                keyframes: scene_analysis.keyframes,
                created_at: Utc::now(),
            };
