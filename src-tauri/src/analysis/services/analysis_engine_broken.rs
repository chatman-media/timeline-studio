...
impl AnalysisEngine {
  ...
  async fn analyze_video_file(
    ...
    for scene in &scenes {
      ...
      if let Ok(emotions) = emotion_detector
        .detect_emotions(&mock_detection)
      {
        ...
        if let Some((_, detection)) = enhanced_detections
          ...
          .find(|(t, _)| *t >= scene.start_time as f64 && *t <= scene.end_time as f64) {
            ...
            let montage_detection = MontageDetection {
              timestamp: scene.start_time as f64,
              detection_type: DetectionType::Combined,
              objects: detection.original_detections.iter().map(|yolo_det| ObjectDetection {
  ...
  async fn analyze_audio_file(
    ...
    let audio_analysis = self.unified_audio_analyzer.analyze_comprehensive(
    ...
    if let Some(ref ffmpeg) = audio_analysis.ffmpeg_analysis {
      ...
      if volume_intensity > 0.7 { // Только если достаточно высокая громкость
        let moment = KeyMoment {
          ...
          description: format!("Audio peak detected with high volume"),
  ...
  async fn analyze_persons_in_file(&self, file: &AnalysisMediaFile) -> Result<Vec<Uuid>> {
    ...
    match file.media_type {
      MediaType::Video | MediaType::Image => {
        ...
        // Пока создаем mock результат - пустой цикл
        let _mock_analysis_results: Vec<i32> = vec![];
              {
                Ok(similar_persons) => {
                  if let Some((person, similarity)) = similar_persons.first() {
                    if *similarity > 0.8 {
                      ...
                      let new_person = crate::recognition::person_database::PersonProfile {
                        id: uuid::Uuid::new_v4(),
                        primary_name: format!("Person_{}", uuid::Uuid::new_v4()),
                      ...
                      // Удаляем старую структуру PersonProfile
                      /*
                      let _old_person = PersonProfile {
                        id: uuid::Uuid::new_v4().to_string(),
                        name: format!("Person_{}", uuid::Uuid::new_v4()),
                        nickname: None,
                      ...
                      */
                      ...
                      match self.person_db.create_person(new_person.clone()).await {
                        Ok(_) => {
                          found_persons.push(new_person.id.to_string());
                    ...
                    let _old_person = PersonProfile {
                      id: uuid::Uuid::new_v4().to_string(),
                      name: format!("Unknown_Person_{}", uuid::Uuid::new_v4()),
                      nickname: None,
                      metadata: PersonMetadata {
                        total_appearances: 1,
                        first_seen: chrono::Utc::now(),
