...
impl AnalysisFrameIntegrator {
    ...
    pub async fn analyze_video_file(
        ...
        info!("Starting comprehensive video analysis for: {:?}", video_path);
        
        // 1. Получаем информацию о видео
        let video_info = self.frame_extractor.preview_generator
            .get_video_info(video_path)
            .await
            .context("Failed to get video info")?;
        
        let duration = video_info.duration;
        info!("Video duration: {:.2}s, analyzing with optimized frame extraction", duration);
        ...
        let object_frames = self.extract_frames_for_object_analysis(
            video_path,
            duration,
            frames_interval,
        ...
        info!("Extracted {} frames for object analysis", object_frames.len());
        ...
        let face_frames = if config.detailed_analysis {
            ...
            self.extract_frames_for_face_analysis(video_path, duration, frames_interval * 0.5).await?
        } else {
        ...
        let scenes = self.detect_scenes_from_objects(&object_detections, duration).await?;

        // 8. Определяем ключевые моменты
        let key_moments = self.detect_key_moments_from_analysis(
            &object_detections,
            &face_detections,
            &scenes,
            duration,
        ...
        info!("Video analysis completed: {} scenes, {} objects, {} faces, {} persons, {} key moments",
            result.scenes.len(),
            result.objects.len(),
            result.faces.len(),
            result.persons.len(),
    ...
    async fn analyze_objects_on_frames(
        ...
        for (frame_index, frame) in frames.iter().enumerate() {
            match self.real_engine.detect_objects_on_frame(&frame.frame_data).await {
                Ok(detections) => {
        ...
        info!("YOLO analysis completed: {} objects detected", results.len());
        Ok(results)
    }
    ...
    async fn analyze_faces_on_frames(
        ...
        // Анализируем каждый кадр
        for (frame_index, frame) in frames.iter().enumerate() {
            match self.real_engine.detect_and_encode_faces(&frame.frame_data).await {
                Ok(face_results) => {
        ...
        info!("FaceNet analysis completed: {} faces detected", results.len());
        Ok(results)
    }
    ...
    async fn detect_scenes_from_objects(
        ...
        info!("Detecting scenes from {} object detections", object_detections.len());
        ...
        for window_idx in 0..num_windows {
            ...
            scenes.push(SceneDetectionResult {
                ...
                dominant_objects: dominant_objects.into_iter().map(|(k, v)| k).collect(),
                object_count: window_objects.len(),
                activity_level,
            });
        }

        info!("Scene detection completed: {} scenes detected", scenes.len());
        Ok(scenes)
    }
    ...
    async fn detect_key_moments_from_analysis(
        ...
        let object_activity_moments = self.detect_high_activity_moments(object_detections, duration).await?;
        ...
        info!("Key moment detection completed: {} moments detected", key_moments.len());
        Ok(key_moments)
    }
    ...
    async fn detect_high_activity_moments(
        ...
        for window_idx in 0..num_windows {
            ...
            if window_objects.len() > 10 { // Порог активности
                ...
                moments.push(KeyMomentResult {
                    ...
                    involved_objects: window_objects.iter().map(|o| o.object_class.clone()).collect(),
    ...
    async fn detect_emotional_peak_moments(
        ...
        for face in faces {
            ...
            if max_emotion_score > 0.8 { // Высокий эмоциональный score
                let emotion_type = face.emotion_scores
                    .iter()
                ...
                moments.push(KeyMomentResult {
                    ...
                    description: format!("Emotional peak: {} (score: {:.2})", emotion_type, max_emotion_score),
        ...
        for window in scenes.windows(2) {
            ...
            if prev_scene.scene_type != current_scene.scene_type {
                moments.push(KeyMomentResult {
                    ...
                    description: format!("Scene transition from {} to {}", prev_scene.scene_type, current_scene.scene_type),
    ...
    async fn deduplicate_moments(
        &self,
        mut moments: Vec<KeyMomentResult>,
        min_interval: f64,
    ) -> Vec<KeyMomentResult> {
        if moments.is_empty() {
            return moments;
    ...
    async fn cluster_persons_from_faces(
        ...
        for face in faces {
            ...
            for cluster in &mut clusters {
                ...
                let cluster_embedding = self.calculate_average_embedding(&cluster.face_embeddings).await?;
                
                // Вычисляем similarity с текущим лицом
                let similarity = self.calculate_embedding_similarity(&face.face_embedding, &cluster_embedding).await?;
            ...
            if !found_cluster {
                ...
                clusters.push(PersonClusterResult {
                    ...
                    estimated_demographics: PersonDemographics {
                        age_range: face.estimated_age.map(|age| (age.saturating_sub(5), age + 5)),
        ...
        for cluster in &mut clusters {
            ...
            let avg_confidence = cluster.appearances.iter()
                .map(|app| app.confidence as f64)
                .sum::<f64>() / cluster.appearances.len() as f64;
            cluster.confidence = avg_confidence;
        }
        
        info!("Person clustering completed: {} unique persons found", clusters.len());
        Ok(clusters)
    }
    ...
    async fn calculate_embedding_similarity(&self, embedding1: &[f32], embedding2: &[f32]) -> Result<f32> {
    ...
    async fn calculate_quality_metrics(&self, frames: &[RecognitionFrame]) -> Result<QualityMetrics> {
        ...
        let avg_resolution = frames.iter()
            .map(|f| f.resolution.0 * f.resolution.1)
            .sum::<u32>() as f64 / frames.len() as f64;
        ...
        let overall_score = (sharpness + stability_score + brightness + contrast + color_balance + noise_level) / 6.0;
    ...
    pub async fn analyze_clip(
        ...
        let video_path = match &clip.source {
            ...
            _ => return Err(anyhow::anyhow!("Only file sources are supported for analysis")),
        };
        ...
        let frames = self.frame_extractor.extract_frames_for_clip(clip, None).await?;
        
        // Преобразуем в RecognitionFrame формат
        let recognition_frames: Vec<RecognitionFrame> = frames.into_iter().map(|frame| {
            RecognitionFrame {
                timestamp: frame.timestamp,
