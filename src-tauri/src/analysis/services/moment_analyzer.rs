...
impl MomentAnalyzer {
  ...
  fn calculate_moments_for_scene(&self, scene: &AnalysisScene) -> usize {
    ...
    let quality_bonus = if scene.quality_score > 0.8 { 1 } else { 0 };
  ...
  fn calculate_importance_score(&self, scene: &AnalysisScene, timestamp: f32) -> f32 {
    ...
    if !(0.2..=0.8).contains(&position_in_scene) {
      score += 0.1;
    }
  ...
  fn create_key_moment(
    ...
    Ok(KeyMoment {
      ...
      scoring_factors: ScoringFactors {
        ...
        object_interest: if scene.objects_detected.is_empty() {
          0.3
        } else {
          0.7
        },
      ...
      is_bookmarked: false,
      is_hidden: false,
      thumbnail_frame: scene.representative_frame,
  ...
  pub fn get_moments_statistics(&self, moments: &[KeyMoment]) -> MomentsStatistics {
    ...
    stats.highest_importance = moments
      .iter()
    ...
    let total_timeline_duration: f32 = moments
      .iter()
