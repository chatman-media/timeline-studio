...
impl ContentEngine {
    ...
    fn calculate_classification_confidence(&self, scenes: &[SceneAnalysis]) -> f64 {
        ...
        let avg_confidence: f64 = scenes.iter().map(|s| s.confidence).sum::<f64>() / scenes.len() as f64;
        avg_confidence
    }
    ...
    pub async fn analyze_composition(&self, visual: &VisualCharacteristics) -> Result<CompositionScore> {
