...
pub async fn analyze_audio_unified(
    ...
    let analysis_config = match config {
        Some(config_str) => {
            match serde_json::from_str::<UnifiedAudioConfig>(&config_str) {
                Ok(config) => config,
                Err(e) => {
    ...
    match analyzer.analyze_comprehensive_with_config(&path, analysis_config).await {
        Ok(result) => {
...
#[command]
pub async fn analyze_audio_quick(
    file_path: String,
) -> Result<String, String> {
    ...
    match analyzer.analyze_basic_metrics(&path).await {
        Ok(basic_metrics) => {
            match serde_json::to_string(&basic_metrics) {
                Ok(json_str) => {
...
pub async fn analyze_audio_with_fallback(
    ...
    info!("Starting fallback audio analysis for: {} with engines: {:?}", file_path, preferred_engines);
        ...
        Ok(result) => {
            match serde_json::to_string(&result) {
                ...
                Err(e) => {
                    ...
                    Err(format!("Failed to serialize fallback analysis result: {}", e))
                }
            }
        }
        Err(e) => {
...
pub async fn get_recommended_audio_config(
    ...
    info!("Getting recommended audio config for: {} (mode: {})", file_path, performance_mode);
...
pub async fn analyze_audio_batch(
    ...
    info!("Starting batch audio analysis for {} files", file_paths.len());
...
pub async fn benchmark_unified_audio_analysis(
    ...
    info!("Running unified audio analysis benchmark: {} iterations", iterations);
...
pub async fn analyze_audio_transcription_unified(
    ...
    match analyzer.analyze_comprehensive_with_config(&path, config).await {
        Ok(result) => {
