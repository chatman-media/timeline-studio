...
use crate::analysis::types::{
    AudioFloat, AudioDuration, AudioVolume, AudioFrequency,
    AudioSampleRate, AudioBasicMetrics, AudioFFmpegAnalysis,
    UnifiedVolumeAnalysis, UnifiedFrequencyAnalysis, UnifiedDynamicsAnalysis,
    UnifiedQualityAnalysis, VolumeHistogramBin, FrequencyDistribution,
    AudioQualityIssue, AudioIssueType, AudioIssueSeverity,
};
...
impl UnifiedFFmpegAudioAnalyzer {
    ...
    pub async fn analyze_audio_comprehensive(
        ...
        info!("Starting comprehensive FFmpeg audio analysis for: {:?}", file_path);
        ...
        let quality_analysis = Self::analyze_quality_unified(file_path, enable_advanced_analysis).await?;
    ...
    pub async fn extract_basic_metrics(file_path: &Path) -> Result<AudioBasicMetrics> {
        ...
        let probe_data: serde_json::Value = serde_json::from_str(&stdout_str)
        ...
        let overall_volume = if has_audio {
            Self::quick_volume_estimation(file_path).await.unwrap_or(AudioVolume::from_normalized(0.5))
        } else {
            AudioVolume::min()
        };
    ...
    async fn analyze_quality_unified(
        ...
        info!("Analyzing audio quality with unified types (advanced: {})", enable_advanced);
    ...
    async fn quick_volume_estimation(file_path: &Path) -> Result<AudioVolume> {
        let output = FFmpegCommand::ffmpeg()
            .args(vec![
                ...
                "-f", "null",
                "-",
                "-t", "10" // Analyze only first 10 seconds for speed
            ])
            .execute()
            .await?;
    ...
    /// Calculate basic quality score
    fn calculate_basic_quality_score(sample_rate: u32, channels: u32, bitrate: Option<u32>) -> AudioFloat {
        let mut score = 0.0;
    ...
    async fn analyze_noise_level(file_path: &Path) -> Result<AudioFloat> {
        ...
        let output = FFmpegCommand::ffmpeg()
            .args(vec![
                "-i", &file_path.to_string_lossy(),
                "-af", "highpass=f=80,lowpass=f=8000,astats",
                "-f", "null",
                "-"
            ])
            .execute()
...
mod tests {
    ...
    async fn test_basic_quality_score_calculation() {
        // High quality
        let score = UnifiedFFmpegAudioAnalyzer::calculate_basic_quality_score(
            48000, 2, Some(256)
        );
        assert!(score > 0.8);
    ...
    fn test_overall_quality_score_calculation() {
        // Perfect audio
        let score = UnifiedFFmpegAudioAnalyzer::calculate_overall_quality_score(
            false, 0.05, 0.02, 30.0
        );
        assert!(score > 0.9);
