...
const logger = createLogger("PreviewEffects")
  ...
  private async applyEffectsInternal(params: PreviewEffectsInput, currentMedia: any): Promise<PreviewEffectsResult> {
    ...
    for (const effect of effects) {
      try {
        ...
        logger.debug(`Применение эффекта ${effect.effectId}`, { parameters: effect.parameters })
      } catch (error) {
        logger.warn("Ошибка применения эффекта", { effectId: effect.effectId, error })
      }
    }
  ...
  private async applyFiltersInternal(params: PreviewEffectsInput, currentMedia: any): Promise<PreviewEffectsResult> {
    ...
    for (const filter of filters) {
      try {
        ...
        logger.debug(`Применение фильтра ${filter.effectId}`, { parameters: filter.parameters })
      } catch (error) {
        logger.warn("Ошибка применения фильтра", { filterId: filter.effectId, error })
      }
    }
