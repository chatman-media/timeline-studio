...
import type {
  ...
  DraggableItem,
  DropTarget,
} from "@/features/drag-drop/services/drag-drop-manager";
...
export const TransitionDropZone = memo(function TransitionDropZone({
  ...
  const canApplyTransitionBetweenClips = useCallback(
    (_transition: Transition): boolean => {
      ...
      const isLeftCompatible =
        leftClip.type === "video" || leftClip.type === "image";
      const isRightCompatible =
        rightClip.type === "video" || rightClip.type === "image";
    ...
    (item: DraggableItem, event: DragEvent) => {
      ...
      const duration =
        typeof transition.duration === "number"
          ? transition.duration
          : transition.duration?.default || 1.0;
  ...
  const handleDragEnter = useCallback(
    (item: DraggableItem) => {
      ...
      if (item.type === "transition") {
        const canAccept = canApplyTransitionBetweenClips(
          item.data as Transition,
        );
  ...
  const hasExistingTransition =
    leftClip.transitions.some((t) => t.type === "out") ||
    rightClip.transitions.some((t) => t.type === "in");
  ...
  return (
    <div
      ...
      title={
        hasExistingTransition ? "Переход применен" : "Перетащите переход сюда"
      }
    >
...
export const AppliedTransition = memo(function AppliedTransition({
  ...
  const transitionName =
    outTransition?.transitionId || inTransition?.transitionId || "Переход";
