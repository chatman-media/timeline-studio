...
|---------|-----------|----------|
| `get_media_files` | `{ directory: string }` | Получить список медиафайлов в директории (используется для импорта директории) |

**Примечание:** Модуль Browser также использует общие медиа-команды из `/src/features/media/`:
- `import_media` - Импорт медиафайлов в проект
- `add_imported_media` - Добавление импортированных медиа в медиапул
- `get_media_metadata` - Получение метаданных медиафайлов (длительность, разрешение и т.д.)
- `clear_imported_media` - Очистка импортированных медиа из состояния backend

Эти команды используются через хук `use-music-import.ts` для функциональности вкладки Music.
