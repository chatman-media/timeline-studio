...
export function useEffectsImport() {
  ...
  const importEffectsFile = useCallback(async (): Promise<ImportResult> => {
    void logger.info("Starting effects file import")
    ...
    try {
      ...
      try {
        ...
        void logger.error("Error reading effects file", { error })
        setIsImporting(false)
        return {
          success: false,
      ...
      void logger.info(`Effects import completed: ${imported} imported, ${failed} failed`)
      return {
        success: true,
        message,
        effects,
      ...
      void logger.error("Error during effects import", { error })
      setIsImporting(false)
      return {
        success: false,
  ...
  const importEffectFile = useCallback(async (): Promise<ImportResult> => {
    void logger.info("Starting effect file import")
    ...
    try {
      ...
      importedEffects.forEach((effect) => {
        try {
          ...
          void logger.error(`Failed to register LUT effect ${effect.id}`, { error })
        }
      })
      ...
      void logger.info(`Imported ${imported} effect files`)
      return {
        success: true,
      ...
      void logger.error("Error during effect files import", { error })
      setIsImporting(false)
      return {
        success: false,
        message: `Ошибка при импорте: ${String(error)}`,
