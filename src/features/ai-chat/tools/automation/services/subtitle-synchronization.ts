...
const logger = createLogger({ module: "SubtitleSynchronization" })
...
export class SubtitleSynchronizationService {
  ...
  public async synchronizeSubtitles(
    ...
    try {
      ...
      logger.info(`Synchronization completed: ${synchronized.length} subtitles, quality score: ${quality.overallScore}`)
  ...
  private applyAIRefinements(
    ...
    return subtitles.map((subtitle, _index) => {
      ...
      if (wordCount > options.maxWordsPerSubtitle!) {
        ...
        logger.warn(`Subtitle too long: ${wordCount} words`)
      }
