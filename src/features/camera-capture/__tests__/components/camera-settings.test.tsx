...
describe("CameraSettings", () => {
  ...
  it("calls onFrameRateChange when frame rate select changes", () => {
    ...
    expect(mockOnFrameRateChange).toHaveBeenCalledWith(NaN); // В тесте мы передаем "test-value", которое не преобразуется в число
  });
  ...
  it("shows loading state when isLoadingCapabilities is true", () => {
    ...
    const loadingElements = screen.getAllByText(
      "dialogs.cameraCapture.determiningCapabilities",
    );
