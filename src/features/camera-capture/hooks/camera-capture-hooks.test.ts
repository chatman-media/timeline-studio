...
import { COMMON_FRAMERATES, COMMON_RESOLUTIONS } from "@/types/project";

import {
  useCameraPermissions,
  useDeviceCapabilities,
} from "./camera-capture-hooks";
...
describe("useDeviceCapabilities", () => {
  ...
  it("should use common resolutions when getCapabilities is not supported", async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockSetSelectedResolution).toHaveBeenCalledWith(
        COMMON_RESOLUTIONS[0].value,
      );
