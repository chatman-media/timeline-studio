...
export class SceneContextService {
  ...
  private generateSceneDescription(
    ...
    Object.entries(objectCounts).forEach(([className, count]) => {
      const objectsOfClass = detectedObjects.filter(
      ...
      if (count === 1) {
        ...
        if (uniquePositions.length === 1) {
          ...
          descriptions.push(
            `${count} ${className}(ов) ${uniquePositions.join(" и ")}`,
          );
        } else {
  ...
  public createChatDescription(context: AISceneContext): string {
    ...
    const classDescriptions = Object.entries(objectCounts).map(
      ([className, count]) => {
  ...
  public filterByClass(
    context: AISceneContext,
    targetClass: string,
  ): AISceneContext {
    const filteredObjects = context.detectedObjects.filter(
      (obj) => obj.class === targetClass,
    );
    ...
    return {
      ...
      dominantObjects: context.dominantObjects.includes(targetClass)
        ? [targetClass]
        : [],
      sceneDescription: this.generateSceneDescription(
        filteredObjects,
        filteredCounts,
      ),
    };
