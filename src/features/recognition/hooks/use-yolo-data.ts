  ...
  const loadYoloData = useCallback(
    async (
      ...
      try {
        ...
        console.error(
          `[useYoloData] Ошибка загрузки данных для видео ${videoId}:`,
          error,
        );
        return null;
      } finally {
  ...
  const getYoloDataAtTimestamp = useCallback(
    async (videoId: string, timestamp: number): Promise<YoloDetection[]> => {
      try {
        ...
        console.error(
          `[useYoloData] Ошибка получения данных для времени ${timestamp}:`,
          error,
        );
        return [];
      }
  ...
  const getVideoSummary = useCallback(
    async (videoId: string): Promise<YoloVideoSummary | null> => {
      try {
        ...
        console.error(
          `[useYoloData] Ошибка получения сводки для видео ${videoId}:`,
          error,
        );
        return null;
      }
  ...
  const getAllYoloData = useCallback(
    async (videoId: string): Promise<YoloVideoData | null> => {
      try {
        ...
        console.error(
          `[useYoloData] Ошибка получения всех данных для видео ${videoId}:`,
          error,
        );
        return null;
      }
    ...
    async (videoIds: string[]): Promise<void> => {
      const promises = videoIds.map((videoId) =>
        ...
        loadYoloData(videoId).catch((error) => {
          console.warn(
            `[useYoloData] Не удалось предзагрузить данные для видео ${videoId}:`,
            error,
          );
        }),
      );
