...
export class AdvancedTrackingService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      await getAI().initAdvancedTracking(this.config)
  ...
  async startTracking(videoId: string): Promise<void> {
    ...
    try {
      await getAI().startPersonTracking({
        videoId,
        config: this.config,
      })
      ...
      logger.debug(`Трекинг запущен для видео ${videoId}`)
    } catch (error) {
  ...
  async processFrame(
    ...
    try {
      ...
      const result = await getAI().processTrackingFrame({
  ...
  async predictNextPositions(): Promise<Map<string, BoundingBox>> {
    ...
    try {
      ...
      const predictedBoxes = await getAI().predictTrackPositions(trackIds)
  ...
  async assignPersonToTrack(trackId: string, personId: string): Promise<void> {
    ...
    try {
      await getAI().assignPersonToTrack(trackId, personId)
  ...
  async mergeTracks(sourceTrackId: string, targetTrackId: string): Promise<void> {
    ...
    try {
      await getAI().mergeTracks(sourceTrackId, targetTrackId)
  ...
  async interpolateMissingPositions(trackId: string, startFrame: number, endFrame: number): Promise<TrajectoryPoint[]> {
    ...
    try {
      const interpolated = await getAI().interpolateTrackPositions({
        trackId,
        startFrame,
        endFrame,
      })
  ...
  async stopTracking(): Promise<void> {
    ...
    try {
      await getAI().stopPersonTracking()

      this.isTracking = false

      // Сохраняем финальное состояние треков
      const finalTracks = this.getActiveTracks()
      logger.debug(`Трекинг остановлен. Финальных треков: ${finalTracks.length}`)
  ...
  async updateConfig(config: Partial<TrackingConfig>): Promise<void> {
    ...
    if (this.isInitialized) {
      await getAI().updateTrackingConfig(this.config)
    }
  }
