...
| src/features/app-state/testing/mock-backend-provider.tsx | MockBackendProvider | test harness w/ `executeCommand` mock & createTestScenarios | Testing harness | ✅ **VERIFIED** - Comprehensive test framework ready for all provider integration tests | High |
| src/features/user-settings/services/user-settings-provider.tsx | UserSettingsProvider / useUserSettings | uses `getProjectManagementOrchestrator()` (orchestrator) | Orchestrator-backed (verified) | ✅ **VERIFIED** - ProjectManagementOrchestrator creates `appActor` and sends `EXECUTE_COMMAND` to it — provider is compliant. MockBackendProvider tests added. | High |
| src/domains/project-management/providers/project-management-provider.tsx | ProjectManagementProvider | returns `getProjectManagementOrchestrator()` | Orchestrator wrapper (verified) | ✅ **VERIFIED** - Uses ProjectManagementOrchestrator which creates `appActor` and routes commands via `EXECUTE_COMMAND` — provider is compliant. Integration tests added. | High |
| src/features/video-player/services/player-provider.tsx | PlayerProvider / usePlayer | uses `getBackendSync()`, calls `executeCommand`, subscribes to backend state | Backend-integrated | ✅ **VERIFIED** - Comprehensive integration tests created with MockBackendProvider. Backend sync working correctly, local state properly isolated. | High |
...
| src/domains/video-editing/providers/video-editing-provider.tsx | VideoEditingProvider | `getVideoEditingOrchestrator()` | Orchestrator-backed (verified) | ✅ **VERIFIED** - VideoEditingOrchestrator subscribes to `backendSync.onStateChange` and uses `backendSync.executeCommand()` for project commands — provider is compliant. Integration tests added. | High |
