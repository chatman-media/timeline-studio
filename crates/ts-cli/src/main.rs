...
use serde::Serialize;
use ts_agent::llm_planner::{LlmPlanParams, LlmPlanner};
...
#[command(
    name = "timeline",
    version,
    about = "Headless Timeline Studio CLI для агента"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
enum PublishPlatform {
    ...
    Youtube {
        ...
        #[arg(long)]
        validate_only: bool,
        /// Вывести machine-readable JSON вместо human-readable текста
        #[arg(long)]
        json: bool,
    },
...
async fn main() {
    ...
    match Cli::parse().cmd {
        ...
        Cmd::Models { action } => match action {
            ...
            ModelsAction::Download { model, all, dir } => {
                cmd_models_download(model, all, dir).await
            }
        },
    }
}
        ...
        match publisher.validate_token(&token).await {
            Ok(channel) => {
                if json {
                    ...
                    println!("ok token valid, channel: {channel}");
                }
            }
            Err(e) => {
        ...
        match publisher.validate_token(&token).await {
            Ok(name) => {
                if json {
                    ...
                    println!("✅ токен действителен, бот: {name}");
                }
            }
            Err(e) => {
...
async fn cmd_montage_plan(
    ...
    eprintln!(
        "Building montage plan (style={style}, duration={duration}s, platform={platform})..."
    );
...
/// Скачать ONNX-модели.
async fn cmd_models_download(model: Option<String>, all: bool, dir: Option<std::path::PathBuf>) {
    let b = match dir {
