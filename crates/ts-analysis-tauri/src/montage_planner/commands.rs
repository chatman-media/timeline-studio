...
use crate::commands::ai_director_commands::AIDirectorState;
use crate::services::ai_director::{AIDirectorConfig, ComprehensiveAnalysisResult};
...
pub async fn analyze_montage_videos(
  ...
  for (index, video_id) in video_ids.iter().enumerate() {
    ...
    match crate::commands::ai_director_commands::ai_director_analyze_comprehensive(
      video_id.clone(),
      Some(config.clone()),
      ai_director.clone(),
    )
