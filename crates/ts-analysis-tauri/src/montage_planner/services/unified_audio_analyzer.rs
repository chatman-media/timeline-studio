...
impl UnifiedMontageAudioAnalyzer {
  ...
  async fn extract_basic_metrics<P: AsRef<Path>>(
    ...
    Ok(AudioBasicMetrics {
      ...
      sample_rate: crate::types::AudioSampleRate { hz: sample_rate_hz },
      channels,
      overall_volume,
      estimated_quality,
  ...
  pub fn to_legacy_montage_analysis(
    ...
    let beat_detection = result.tempo.map(|tempo| {
      crate::types::AudioBeatAnalysis {
        bpm: tempo,
        beat_times: result
          .beat_markers
          .iter()
          .map(|&t| crate::types::AudioTimestamp::from_seconds(t))
          .collect(),
    ...
    let tempo_analysis = result.tempo.map(|tempo| {
      crate::types::AudioTempoAnalysis {
        average_tempo: tempo,
...
mod tests {
  ...
  fn test_quality_score_calculation() {
    ...
    let basic_metrics = AudioBasicMetrics {
      ...
      sample_rate: crate::types::AudioSampleRate { hz: 48000 },
      channels: 2,
  ...
  fn test_legacy_conversion() {
    ...
    let unified_result = UnifiedMontageAudioAnalysisResult {
      ...
      basic_metrics: AudioBasicMetrics {
        has_audio: true,
        duration: AudioDuration::from_seconds(30.0),
        sample_rate: crate::types::AudioSampleRate { hz: 44100 },
        channels: 2,
