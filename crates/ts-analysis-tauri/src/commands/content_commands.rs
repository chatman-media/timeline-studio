...
use crate::engines::content_engine::{
  CompositionScore, CompositionWeights, ContentClassification, ContentEngine, ContentEngineConfig,
  QualityScore,
};
use crate::types::unified_types::{SceneAnalysis, VisualCharacteristics};
use crate::types::MoodAnalysis;
use anyhow::Result;
  ...
  for scene in scenes {
    if let Some(visual) = scene.visual {
      match engine.analyze_composition(&visual).await {
        ...
        Err(e) => {
          log::warn!(
            "Failed to analyze composition for scene {}: {}",
            scene.id,
            e
          );
        }
...
pub async fn analyze_content_comprehensive(
  ...
  log::info!(
    "Performing comprehensive content analysis for {} scenes",
    scenes.len()
  );
...
mod tests {
  use super::*;
  use crate::types::unified_types::{AudioCharacteristics, SceneType};
