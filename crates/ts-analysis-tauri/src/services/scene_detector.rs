...
use crate::models::*;
// use ts_render_analysis::audio_analysis::FFmpegAudioAnalyzer;  // Временно отключено
// use ts_render_analysis::video_analysis::FFmpegVideoAnalyzer;  // Временно отключено
    ...
    for (i, &start_time) in scene_changes.iter().enumerate() {
      ...
      let scene = AnalysisScene {
        ...
        emotional_tone: scene_analysis.emotional_tone.map(|tone| {
          ts_recognition::recognition::types_professional::EmotionData {
