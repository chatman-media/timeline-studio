...
use crate::services::real_analysis_engine::{AnalysisEngineConfig, RealAnalysisEngine};
use ts_render::video_compiler::cache::RenderCache;
use ts_render::video_compiler::core::frame_extraction::{
...
impl AnalysisFrameIntegrator {
  ...
  pub fn with_cache(cache: Arc<RwLock<RenderCache>>) -> Self {
    ...
    let person_db = Arc::new(ts_recognition::recognition::person_database::PersonDatabase::new_mock());
    let analysis_db = Arc::new(crate::database::AnalysisDatabase::new_mock());
    let project_manager = Arc::new(crate::services::ProjectManager::new(
      analysis_db.clone(),
    ));
  ...
  async fn extract_frames_for_object_analysis(
    ...
    let _settings = ExtractionSettings {
      ...
      quality: 85,
      _format: ts_render::video_compiler::schema::PreviewFormat::Png,
  ...
  pub async fn analyze_clip(
    ...
    let _video_path = match &clip.source {
      ts_render::video_compiler::schema::ClipSource::File(path) => std::path::Path::new(path),
      _ => {
