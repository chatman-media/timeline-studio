...
use ts_recognition_tauri::commands::yolo_commands::YoloProcessorState;
use ts_recognition::recognition::person_database::PersonDatabase;
use ts_recognition::recognition::types_professional::Emotion;
use ts_state_tauri::project_state::MediaType;
// use ts_recognition::recognition::frame_processor::YoloDetection; // Временно отключено
  ...
  async fn analyze_video_file(
    ...
    for scene in &scenes {
      ...
      if let Ok(emotions) = emotion_detector.detect_emotions(&mock_detection) {
        ...
        enhanced_scene.emotional_tone = Some(ts_recognition::recognition::types_professional::EmotionData {
      ...
      if composition_change > 0.3 || i == detections.len() - 1 {
        ...
        if scene_end - current_scene_start > 1.0 {
          ...
          let scene = AnalysisScene {
            ...
            emotional_tone: Some(ts_recognition::recognition::types_professional::EmotionData {
