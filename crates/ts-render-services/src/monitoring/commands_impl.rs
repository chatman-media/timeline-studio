//! Pure-logic implementations for monitoring commands (no Tauri, no State wrapper)
...
use crate::services::monitoring::MetricsSummary;
...
pub async fn reset_all_metrics(state: &VideoCompilerState) -> Result<()> {
...
pub async fn reset_progress_tracker(state: &VideoCompilerState) -> Result<bool> {
