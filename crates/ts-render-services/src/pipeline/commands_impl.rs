//! Реализация команд pipeline — принимают &VideoCompilerState
...
pub async fn cancel_pipeline(job_id: String, state: &VideoCompilerState) -> Result<()> {
  let pipelines = state.active_pipelines.read().await;
...
pub async fn get_active_pipelines(state: &VideoCompilerState) -> Result<Vec<String>> {
  let pipelines = state.active_pipelines.read().await;
  Ok(pipelines.keys().cloned().collect())
}

/// Очистить завершенные конвейеры
pub async fn cleanup_finished_pipelines(state: &VideoCompilerState) -> Result<usize> {
  let mut pipelines = state.active_pipelines.write().await;
...
pub async fn cleanup_completed_pipelines(state: &VideoCompilerState) -> Result<usize> {
