  ...
  async fn test_span_creation() {
    ...
    let result = tracer
      .trace("test_operation", async {
        ...
        Ok::<_, ts_render::video_compiler::error::VideoCompilerError>(42)
      })
      .await;
      ...
      .trace("parent_operation", async {
        ...
        let child1_result = tracer
          .trace("child_operation_1", async {
            ...
            Ok::<i32, ts_render::video_compiler::error::VideoCompilerError>(10)
          })
          .await?;
        ...
        let child2_result = tracer
          .trace("child_operation_2", async {
            ...
            Ok::<i32, ts_render::video_compiler::error::VideoCompilerError>(20)
          })
          .await?;
          ...
          .trace("deep_operation", async {
            tracer
              .trace("deepest_operation", async {
                ...
                Ok::<i32, ts_render::video_compiler::error::VideoCompilerError>(5)
              })
              .await
          })
          .await?;
  ...
  async fn test_span_with_attributes() {
    ...
    let result = tracer
      ...
      .run(async {
        ...
        Ok::<String, ts_render::video_compiler::error::VideoCompilerError>("response_data".to_string())
      })
      .await;
  ...
  async fn test_span_with_error_handling() {
    ...
    let result = tracer
      .trace("failing_operation", async {
        tokio::time::sleep(Duration::from_millis(5)).await;
        Err::<i32, ts_render::video_compiler::error::VideoCompilerError>(
  ...
  async fn test_span_builder_kinds() {
    ...
    for kind in kinds {
      let result = tracer
        .span("test_span")
        .with_kind(kind)
        .run(async { Ok::<_, ts_render::video_compiler::error::VideoCompilerError>("success") })
        .await;
        ...
        tokio::spawn(async move {
          tracer_clone
            .trace(&format!("concurrent_op_{}", i), async {
              tokio::time::sleep(Duration::from_millis(1)).await;
              Ok::<_, ts_render::video_compiler::error::VideoCompilerError>(i)
            })
            .await
        })
      })
