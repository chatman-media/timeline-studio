...
impl TelemetryManager {
  ...
  pub async fn setup_system_health_checks(
    ...
    plugin_manager: Option<Arc<crate::PluginManager>>,
  ) -> Result<()> {
...
mod tests {
  ...
  async fn test_telemetry_manager_system_health_checks() {
    use crate::plugins::plugin::Version;
    use crate::{EventBus, PluginManager, ServiceContainer};
    ...
    for i in 0..total_traces {
      let result = tracer
        .trace(&format!("sample_operation_{i}"), async {
          ...
          Ok::<i32, ts_render::video_compiler::error::VideoCompilerError>(i)
        })
        .await;
    ...
    for sample_rate in configs_to_test {
      ...
      let result = test_tracer
        .trace("sample_rate_test", async {
          Ok::<String, ts_render::video_compiler::error::VideoCompilerError>(format!(
            "rate_{sample_rate}"
          ))
        })
        .await;
    ...
    for i in 0..5 {
      ...
      let result = tracer
        ...
        .run(async {
          ...
          Ok::<String, ts_render::video_compiler::error::VideoCompilerError>(format!("completed_{i}"))
        })
        .await;
  ...
  async fn test_telemetry_manager_partial_system_health_setup() {
    ...
    let service_container = Arc::new(crate::ServiceContainer::new());
    let app_version = crate::plugins::plugin::Version::new(1, 0, 0);
    let event_bus2 = Arc::new(crate::EventBus::new());
    let plugin_manager = Arc::new(crate::PluginManager::new(
      app_version,
      event_bus2,
      service_container,
    ));
