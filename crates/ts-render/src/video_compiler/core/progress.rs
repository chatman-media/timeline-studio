...
impl ProgressTracker {
  ...
  pub async fn create_job(
    ...
    let mut job = RenderJob::new(job_id.clone(), project_name, output_path, total_frames);
    job.start().ok(); // Queued -> Processing (апстримный start() был dead_code)
...
impl RenderJob {
  ...
  pub fn fail(&mut self, error: String) -> Result<()> {
    ...
    self.status = RenderStatus::Failed(error.clone());
    self.completed_at = Some(SystemTime::now());
  ...
  pub fn cancel(&mut self) -> Result<()> {
    if matches!(
      self.status,
      RenderStatus::Completed | RenderStatus::Failed(_)
    ) {
      return Err(VideoCompilerError::render(
        &self.id,
        "cancel",
