...
impl<'a> FilterBuilder<'a> {
  ...
  async fn build_filter_complex(&self) -> Result<String> {
    // Тривиальный случай (один клип, без эффектов/фильтров/субтитров): filtergraph
    // не нужен. Старый билдер генерил невалидный "[track0][outv]" → ffmpeg
    // "No such filter: ''". Пропускаем — ffmpeg просто транскодирует единственный вход.
    let total_clips: usize = self
      ...
      .map(|t| t.clips.len())
      .sum();
    let any_fx = self.project.tracks.iter().any(|t| {
      !t.effects.is_empty()
        || !t.filters.is_empty()
