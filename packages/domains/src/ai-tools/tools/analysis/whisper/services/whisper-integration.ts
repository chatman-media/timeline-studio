...
import type { SpeechDetection } from "@timeline-studio/domains/ai-services/types"
import type { TranscriptionResult, WhisperIntegrationOptions } from "@timeline-studio/domains/ai-services/types/transcription"

import { createLogger } from "@/lib/tauri-logger"

const logger = createLogger("WhisperIntegration")
...
export class WhisperIntegrationService {
  ...
  public async recognizeSpeechWithSpeakers(
    ...
    try {
      ...
      logger.debug("Speech recognition with speakers completed", {
        segments: withSpeakers.length,
        speakers: this.countSpeakers(withSpeakers),
      })
      return withSpeakers
    } catch (error) {
  ...
  public async recognizeForSubtitles(
    ...
    try {
      ...
      logger.info("Optimized speech recognition completed", { count: speechDetections.length })
  ...
  public async checkModelAvailability(): Promise<{ available: string[]; recommended: string }> {
    try {
      ...
      logger.debug("Available Whisper models:", { data: available })
