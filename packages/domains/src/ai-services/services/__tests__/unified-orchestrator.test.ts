...
vi.mock("@timeline-studio/domains/ai-services/utils/validation")
...
vi.mock("@timeline-studio/domains/shared/events", () => ({
  eventBus: {
    subscribe: vi.fn(),
...
describe("UnifiedOrchestrator", () => {
  ...
  describe("Event Publishing", () => {
    it("должен публиковать события начала анализа", async () => {
      const { eventBus } = await import("@timeline-studio/domains/shared/events")
