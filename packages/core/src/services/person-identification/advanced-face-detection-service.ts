...
export class AdvancedFaceDetectionService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      await getAI().initRecognitionYoloProcessor({
      ...
      await getAI().initPersonFacenetProcessor({
        modelType: facenetModel,
      })

      // Проверяем доступность GPU
      if (this.config.useGPU) {
        const gpuAvailable = await getAI().checkGPUAvailability()
        if (!gpuAvailable) {
      ...
      logger.debug("Advanced Face Detection Service инициализирован")
    } catch (error) {
  ...
  async detectFacesAdvanced(
    ...
    try {
      ...
      const results = await getAI().detectFacesAdvanced({
  ...
  async generateFaceEmbedding(
    ...
    try {
      ...
      const result = await getAI().generateFaceEmbeddingFromBase64(`data:image/jpeg;base64,${base64Data}`)
  ...
  async analyzeFaceQuality(faceImage: string | ArrayBuffer): Promise<FaceImageQuality> {
    ...
    try {
      ...
      const quality = await getAI().analyzeFaceQuality(base64Data)
  ...
  async startRealtimeProcessing(
    ...
    try {
      ...
      await getAI().startRealtimeFaceDetection({
        streamId,
        config: this.config,
      })
  ...
  async stopRealtimeProcessing(): Promise<void> {
    ...
    try {
      await getAI().stopRealtimeFaceDetection()
  ...
  async updateConfig(config: Partial<AdvancedDetectionConfig>): Promise<void> {
    ...
    if (this.isInitialized) {
      await getAI().updateFaceDetectionConfig(this.config)
    }
  }
  ...
  private async setupRealtimeListeners(_callbacks: {
    ...
    return () => {
      logger.debug("Realtime listeners cleanup")
    }
  }
  ...
  async blurFaces(
    ...
    try {
      ...
      const blurredImage = await getAI().blurFacesInImageAdvanced({
        imageData: base64Data,
  ...
  async cleanup(): Promise<void> {
    ...
    if (this.isInitialized) {
      await getAI().cleanupFaceDetection()
