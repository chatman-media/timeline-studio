...
export class PersonDatabaseService {
  ...
  private async initializeTauriDB(): Promise<void> {
    try {
      await getAI().initPersonDatabase()
  ...
  async createPerson(personData: Omit<PersonProfile, "id" | "createdAt" | "updatedAt">): Promise<PersonProfile> {
    ...
    if (this.config.storage === "tauri") {
      const person = await getAI().createPersonProfile({
  ...
  async getPerson(personId: string): Promise<PersonProfile | null> {
    ...
    try {
      ...
      if (this.config.storage === "tauri") {
        person = await getAI().getPersonProfile(personId)
      } else {
  ...
  async deletePerson(personId: string): Promise<boolean> {
    ...
    try {
      if (this.config.storage === "tauri") {
        await getAI().deletePersonProfile(personId)
      } else {
  ...
  async searchPersonsByEmbedding(
    ...
    try {
      if (this.config.storage === "tauri") {
        ...
        const searchResults = await getAI().searchSimilarPersonProfiles({
  ...
  async addEmbedding(personId: string, embedding: FaceEmbedding): Promise<boolean> {
    ...
    try {
      if (this.config.storage === "tauri") {
        await getAI().addPersonFaceEmbedding({
          personId,
  ...
  async addAppearance(personId: string, appearance: PersonAppearance): Promise<boolean> {
    ...
    try {
      if (this.config.storage === "tauri") {
        await getAI().addPersonAppearanceRecord({
          personId,
  ...
  async getDatabaseStats(): Promise<DatabaseStats> {
    ...
    try {
      if (this.config.storage === "tauri") {
        const stats = await getAI().getPersonDatabaseStats()
        return {
          totalPersons: stats.totalPersons,
          totalEmbeddings: stats.totalEmbeddings,
          totalAppearances: stats.totalAppearances,
          averageEmbeddingsPerPerson: stats.averageEmbeddingsPerPerson ?? 0,
  ...
  async setSimilarityThreshold(threshold: number): Promise<void> {
    if (this.config.storage === "tauri") {
      await getAI().setPersonSimilarityThreshold(threshold)
    }
  ...
  async addPersonThumbnail(
    ...
    try {
      if (this.config.storage === "tauri") {
        ...
        await getAI().addPersonThumbnailRecord({
          personId,
