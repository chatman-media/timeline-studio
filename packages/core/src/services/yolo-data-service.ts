...
export class YoloDataService {
  ...
  public async loadYoloData(videoId: string, videoPath?: string): Promise<YoloVideoData | null> {
    ...
    try {
      ...
      const savedData = await getAI().loadYoloData(videoId)
      ...
      if (videoPath) {
        ...
        // Анализируем видео
        const results = await getAI().analyzeVideoWithYoloData({
        ...
        const frames: YoloFrameData[] = results.map((result: RecognitionYoloFrameResult) => ({
        ...
        // Сохраняем данные
        await getAI().saveYoloData(videoId, yoloData)
