...
async function executeTauriCommand(command, params = {}) {
  try {
    ...
    return new Promise((resolve, reject) => {
      const process = spawn('bun', ['run', 'tauri', 'invoke', command, JSON.stringify(params)], {
        stdio: 'pipe',
  ...
  try {
    ...
    switch (command) {
      ...
      case 'create_multicam_montage':
        try {
          const result = await executeTauriCommand('create_and_execute_pipeline', {
    ...
    switch (command) {
      case 'analyze_video_quality':
        ...
        return {
          ...
          cameras: [
            { name: 'iPhone_main', quality: 95, resolution: '4K', stability: 'excellent' },
            { name: 'DJI_drone', quality: 90, resolution: '4K', stability: 'good' }
          ]
        };
...
Отвечай на русском языке. Если пользователь просит обработку видео - выполняй соответствующие команды.`;
    ...
    log(`⚠️ Claude API недоступен, использую fallback: ${error.message}`, 'yellow');
    return null; // Fallback к симуляции
  }
}
...
async function getAIResponse(message, context = null, provider = 'claude') {
  ...
  if (aiResponse) {
    ...
    if (responseMsg.includes('анализ') || responseMsg.includes('качество') || responseMsg.includes('камер')) {
      log(`🔬 ИИ предлагает анализ качества, выполняю...`, 'yellow');
      const qualityAnalysis = await executeTimelineCommand('analyze_video_quality');
      return aiResponse + `\n\n🔬 Результат анализа:\n${JSON.stringify(qualityAnalysis, null, 2)}`;
    }
    
    if (responseMsg.includes('дрон') || responseMsg.includes('сегмент') || responseMsg.includes('полет')) {
      log(`🚁 ИИ хочет найти моменты дрона, выполняю...`, 'yellow');
      const droneSegments = await executeTimelineCommand('detect_drone_flight_segments');
      return aiResponse + `\n\n🚁 Найденные сегменты:\n${JSON.stringify(droneSegments, null, 2)}`;
    }
    
    if (responseMsg.includes('монтаж') || responseMsg.includes('создать') || responseMsg.includes('собрать')) {
      log(`🎬 ИИ предлагает создать монтаж, выполняю...`, 'yellow');
      const montageResult = await executeTimelineCommand('create_multicam_montage');
  ...
  if (msg.includes('основн') && msg.includes('камер') && msg.includes('качеств')) {
    log(`🤖 Понял! Нужно найти камеру с лучшим качеством и использовать её как основу`, 'blue');
    ...
    return `✅ Анализ завершен! Лучшая камера: ${bestCamera} (качество: ${qualityAnalysis.cameras[0].quality}/100)\n\n` +
           `📋 План монтажа:\n` +
  ...
  if (msg.includes('дрон') && (msg.includes('момент') || msg.includes('пролет') || msg.includes('вставк'))) {
  ...
  if (msg.includes('монтаж') || msg.includes('создать') || msg.includes('сделать')) {
    log(`🤖 Создаю multicam монтаж согласно твоему плану...`, 'blue');
    
    // Сначала синхронизируем камеры
    const syncResult = await executeTimelineCommand('sync_cameras_by_audio');
    if (!syncResult.error) {
      log(`✅ Камеры синхронизированы по аудио (${syncResult.syncPoints.length} точек)`, 'green');
    }
  ...
  const responses = {
    claude: {
      greeting: "Привет! Я Claude, ваш ИИ-ассистент для работы с Timeline Studio. Могу помочь с анализом видео, созданием монтажа или ответить на вопросы о проекте.",
      analyze: "Для анализа ваших видео с Пхукета я рекомендую использовать YOLO детекцию для поиска интересных объектов (лодки, люди, закаты), анализ аудио для поиска музыкальных моментов, и детекцию сцен для разбивки на смысловые части.",
      montage: "Для создания монтажа из видео с отпуска рекомендую стиль 'travel' с длительностью 2-3 минуты. Использую динамические переходы между сценами, синхронизацию с музыкой и умное кадрирование для акцента на главных объектах.",
      ...
      error: "Извините, произошла ошибка при обработке вашего запроса. Попробуйте переформулировать вопрос."
    },
    openai: {
      greeting: "Здравствуйте! Я GPT-ассистент для Timeline Studio. Готов помочь с обработкой видео и созданием монтажа.",
      analyze: "Рекомендую начать с глубокого анализа контента: детекция объектов, анализ качества, определение лучших моментов. Для видео с Пхукета особенно важна детекция воды, неба и людей.",
      montage: "Создам план монтажа на основе анализа: выберу лучшие кадры, синхронизирую с музыкой, добавлю плавные переходы. Стиль 'travel' идеален для отпускных видео.",
      help: "Я могу помочь с планированием обработки видео, объяснить параметры анализа, подсказать оптимальные настройки монтажа.",
      error: "Не удалось обработать запрос. Попробуйте задать вопрос по-другому."
    }
  };
...
// Интерактивный чат
async function startInteractiveChat(provider = 'claude', model = 'llama3.2:latest') {
  ...
  rl.on('line', async (input) => {
    ...
    if (message === '/history') {
      ...
      if (chatHistory.length === 0) {
        ...
        chatHistory.slice(-10).forEach((entry, index) => {
          log(`  ${index + 1}. [${entry.timestamp}] ${entry.user}: ${entry.message.substring(0, 60)}...`, 'dim');
        });
      }
      log('');
      rl.prompt();
    ...
    try {
      ...
      const recentContext = chatHistory.slice(-5).map(h => h.message).join(' ');
...
program
  ...
  .action(async (question, options) => {
    try {
      ...
      const response = await getAIResponse(question, options.context, options.provider);
...
program
  ...
  .action(async (options) => {
    try {
      log(`🔍 Анализ видео с помощью ${options.provider.toUpperCase()}`, 'green');
      
      // Получаем список файлов
      const videoFiles = fs.readdirSync(options.dir)
      ...
      rl.question('', (answer) => {
        if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
          ...
          log(`💡 Команда: node scripts/cli-video-processor.js analyze -d ${options.dir} --yolo --audio`, 'dim');
        } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        data.models.forEach((model, index) => {
          ...
          log(`   🧮 Параметры: ${model.details?.parameter_size || 'Unknown'}`, 'dim');
          log('');
        });
        
        log(`💡 Использование:`, 'yellow');
        log(`   bun run ai:chat -p llama -m llama3.2:latest`, 'cyan');
        log(`   node scripts/timeline-cli.js chat -p llama -m MODEL_NAME`, 'cyan');
