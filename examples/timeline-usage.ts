...
function createSimpleProject(): TimelineProject {
  ...
  const morningSection = createTimelineSection(
    ...
    new Date("2025-01-15T08:00:00"), // реальное время съемки
  );

  // Добавляем видео трек в секцию
  const videoTrack = createTimelineTrack(
    "Основное видео",
    "video",
    morningSection.id,
  );
  ...
  const audioTrack = createTimelineTrack(
    "Основной звук",
    "audio",
    morningSection.id,
  );
...
function addClipsToProject(project: TimelineProject): void {
  ...
  const audioClip1 = createTimelineClip(
    "media-audio-001",
    audioTrack.id,
    0,
    120,
  ...
  const audioClip2 = createTimelineClip(
    "media-audio-002",
    audioTrack.id,
    120,
    60,
  ...
  const musicClip = createTimelineClip(
    "media-music-001",
    musicTrack.id,
    0,
    300,
...
function demonstrateTimeOperations(project: TimelineProject): void {
  ...
  console.log(
    `Общая длительность: ${secondsToTimecode(totalDuration)} (${totalDuration} сек)`,
  );
...
function validateAndCheckProject(project: TimelineProject): void {
  ...
  if (videoTrack) {
    ...
    console.log(
      `Можно разместить клип на 200 сек длительностью 30 сек: ${canPlace}`,
    );
  }
...
function createComplexProject(): TimelineProject {
  ...
  const sections = [
    createTimelineSection("Утро", 0, 600, new Date("2025-01-15T08:00:00")),
    createTimelineSection("День", 600, 900, new Date("2025-01-15T12:00:00")),
    createTimelineSection("Вечер", 1500, 600, new Date("2025-01-15T18:00:00")),
  ];
...
export function demonstrateTimelineV2(): void {
  ...
  console.log(
    `Сложный проект: ${complexProject.sections.length} секций, ${getAllTracks(complexProject).length} треков`,
  );
